...
class Net(nn.Module):
    """
    The base class which all model and policy networks inherit from. It includes methods
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.welford_state_var_sum = torch.ones(1)
    ...
    @staticmethod
    def _normalize(value, welford_state_count, welford_mean, welford_var_sum, inplace=False):
        variance = torch.div(welford_var_sum, welford_state_count)
        ...
        if inplace:
            ...
            value = torch.div(torch.sub(value, welford_mean), stdev)
        return value
    def normalize_state(self, state, update_norm=False, inplace=False):
        if update_norm:
            self.welford_state_count += 1
        ...
        return self._normalize(value=state, welford_state_count=self.size, welford_mean=self.welford_state_mean, welford_var_sum=self.welford_state_var_sum, inplace=inplace)
    ...
    def copy_welford_state_norm(self, net):
        self.welford_state_count   = net.welford_state_count
        self.welford_state_mean    = net.welford_state_mean
        self.welford_state_var_sum = net.welford_state_var_sum
